...
The backend requires support for multiple database types, transaction management, and asynchronous messaging.
...
| **Graph** | Social Graph (Friends, Mutuals) | **Spring Data Neo4j** | Support for Neo4j. Allows mapping nodes/relationships to Java objects, essential for the *Social Graph Service*. |
...
| **Migrations** | Schema Versioning | **Flyway** | Allows the Relational DB (PostgreSQL) to manage schema changes (tables, foreign keys) reliably across environments. |
...
| **Messaging** | Async Notifications | **Spring AMQP (RabbitMQ)** | Provides abstraction for RabbitMQ. Ideal for decoupling the *Game Completion Trigger* from the *Notification Service*. |
...
| **State Management** | Client-side State | **Zustand** | Lightweight and simpler than Redux. Good for managing global client state like "Current User Session" or "Active Chat Room" without boilerplate. |
...
| Category | Requirement | Recommended Library | Justification |
| :--- | :--- | :--- | :--- |
| **Component Library** | UI Consistency | **shadcn/ui** | Built on top of Tailwind CSS (which is already installed). Provides accessible, customizable components that copy directly into the project, avoiding "fighting the framework." |
...
This ensures everyone (`mvn spring-boot:run`) connects to the same infrastructure configuration.
