...
import { Info, TrendingUp, Users, CheckCircle, XCircle, Star, ChevronDown, ChevronUp } from 'lucide-react';
import { useOrganizerQuality, formatOqsScore, getOqsColorClass, getOqsBgColorClass, getConfidenceLevelInfo } from '@/hooks/useOrganizerQuality';
    ...
    if (variant === 'inline') {
        return (
            <div className="inline-flex items-center gap-2">
                <div className={`flex items-center gap-1 px-2 py-0.5 rounded ${getOqsBgColorClass(oqs.oqsScore)}`}>
                    <Star className={`w-3 h-3 ${getOqsColorClass(oqs.oqsScore)}`} />
                    <span className={`text-sm font-medium ${getOqsColorClass(oqs.oqsScore)}`}>
                        {formatOqsScore(oqs.oqsScore)}
                    </span>
                </div>
                <span className={`text-xs ${confidenceInfo.color}`}>{confidenceInfo.icon}</span>
            </div>
        );
    }
            ...
            <div className="relative">
                ...
                {isExpanded && infoCard && showInfoCard && (
                    <div className="absolute top-full left-0 mt-2 w-80 bg-white rounded-xl shadow-lg border border-gray-200 z-50">
                        <OqsInfoCardContent infoCard={infoCard} onClose={() => setIsExpanded(false)} />
                    </div>
                )}
            </div>
        );
            ...
            <div className={`p-4 ${getOqsBgColorClass(oqs.oqsScore)}`}>
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold text-gray-900">Organizer Quality Score</h3>
                            <p className="text-sm text-gray-600">{displayName || 'Organizer'}</p>
                        </div>
                    </div>
                    <div className="text-right">
                        <div className={`text-3xl font-bold ${getOqsColorClass(oqs.oqsScore)}`}>
                            {formatOqsScore(oqs.oqsScore)}
                        </div>
                ...
                <div className="pt-2 border-t border-gray-100">
                    <div className="flex items-center justify-between text-sm">
                        ...
                        <div className="flex items-center gap-2">
                            <span className={confidenceInfo.color}>{confidenceInfo.icon}</span>
                            <span className="text-gray-900">{oqs.totalGamesHosted} games hosted</span>
                        </div>
                    </div>
                    {oqs.confidenceDescription && (
                        <p className="text-xs text-gray-500 mt-1">{oqs.confidenceDescription}</p>
                    )}
                </div>
            </div>
        ...
        <div className="space-y-4 text-sm">
            ...
            <div className="bg-gray-50 rounded-lg p-3">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="font-medium text-gray-900">Game Completion Rate (60%)</span>
                </div>
                <p className="text-gray-600 text-xs">{infoCard.completionRateDescription}</p>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-3">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="font-medium text-gray-900">Repeat Player Rate (40%)</span>
                </div>
                <p className="text-gray-600 text-xs">{infoCard.repeatRateDescription}</p>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-3">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="font-medium text-gray-900">Confidence: {infoCard.confidenceLevel}</span>
                </div>
                <p className="text-gray-600 text-xs">{infoCard.confidenceDescription}</p>
                {infoCard.gamesForNextLevel > 0 && (
                    <p className="text-emerald-600 text-xs mt-1">
                        Host {infoCard.gamesForNextLevel} more game{infoCard.gamesForNextLevel > 1 ? 's' : ''} to increase confidence
                    </p>
                )}
            </div>
        </div>
