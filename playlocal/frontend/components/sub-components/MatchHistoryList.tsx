...
export function MatchHistoryList({ game }: Readonly<MatchHistoryListProps>) {
  const [gameParticipation, setGameParticipation] =
  ...
  const attendanceDisplay = getAttendanceDisplay(
    gameParticipation?.attendanceStatus,
    gameParticipation?.role
  );

  const isOrganizerUnknownAttendance =
      ...
      <div className="flex justify-between mt-2 text-sm">
        <div className="flex items-center gap-4">
          ...
          <div className={`flex items-center gap-1 ${attendanceDisplay.color}`}>
            ...
            <div>{attendanceDisplay.text}</div>
          </div>
        </div>
