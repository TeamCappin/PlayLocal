...
function TestConsumer() {
  const { user, isAuthenticated, isLoading, error, login, register, logout, refreshUser } =
    useAuth();
  ...
  return (
    <div>
      ...
      <button onClick={() => register("r@b.com", "pw", "Reg User", true, true)} type="button">
        doRegister
      </button>
...
describe("AuthContext / AuthProvider", () => {
  ...
  it("login success: calls authApi.login, stores token, sets user, clears loading", async () => {
    ...
    expect(mockedAuthApi.login).toHaveBeenCalledWith({ email: "a@b.com", password: "pw" });
  ...
  it("login failure with ApiError: sets error to ApiError.message and rethrows", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("err")).toHaveTextContent("Invalid credentials");
    });
  ...
  it("login failure with non-ApiError: sets generic message and rethrows", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("err")).toHaveTextContent("Login failed. Please try again.");
    });
  });
  ...
  it("refreshUser: with token, updates user from getCurrentUser", async () => {
    ...
    expect(mockedAuthApi.getCurrentUser).toHaveBeenCalledTimes(1);
    expect(screen.getByTestId("userEmail")).toHaveTextContent("fresh@example.com");
  });
