...
describe('NotificationsPage', () => {
  test('renders sign-in gate when not authenticated', () => {
    ...
    expect(
      screen.getByText('Sign in to view notifications')
    ).toBeInTheDocument();
  ...
  test('mark as read calls API and updates UI (removes unread indicator + mark button)', async () => {
    ...
    await waitFor(() => {
      expect(
        within(itemRoot).queryByTitle('Mark as read')
  ...
  test('mark all as read calls API and updates all items locally', async () => {
    ...
    expect(
      screen.getByRole('button', { name: /Unread \(0\)/i })
    ).toBeInTheDocument();
  });
  ...
  test('dismiss removes item from list and updates counts', async () => {
    ...
    expect(
      screen.getByRole('button', { name: /Unread \(1\)/i })
    ).toBeInTheDocument();
    expect(screen.getByText('1 unread notification')).toBeInTheDocument();
  });
  ...
  test('loadDismissedFromStorage returns empty set when localStorage has no data', () => {
    ...
    expect(getItemSpy).toHaveBeenCalledWith(
      'playlocal-dismissed-notifications'
    );
