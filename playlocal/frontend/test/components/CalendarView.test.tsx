...
function isoLocal(year: number, monthIndex0: number, day: number, hour24 = 12, min = 0) {
...
describe("CalendarView", () => {
  ...
  it("renders header and Create Game link", () => {
    mockUseAuth.mockReturnValue({ user: { userId: "u1" }, isAuthenticated: true });
    mockUseGames.mockReturnValue({ games: [], isLoading: false });

    render(<CalendarView />);

    expect(screen.getByText("My Calendar")).toBeInTheDocument();
    expect(screen.getByText("View and manage your upcoming games")).toBeInTheDocument();
  ...
  it("maps API games into calendar games and shows them in Upcoming Games (sorted, future only, limit 5)", () => {
    mockUseAuth.mockReturnValue({ user: { userId: "host-1" }, isAuthenticated: true });
  ...
  it("renders day cell links for games and shows '+N more' when more than 2 games on a day", () => {
    ...
    const linkA = screen.getAllByRole("link").find((a) => a.getAttribute("href") === "/games/a");
    const linkB = screen.getAllByRole("link").find((a) => a.getAttribute("href") === "/games/b");
  ...
  it("highlights today's date badge in the grid (rendered as a rounded emerald circle)", () => {
    ...
    const todayBadge = Array.from(screen.getAllByText("9")).find((el) =>
      el.className.includes("bg-emerald-600") && el.className.includes("rounded-full")
    );
