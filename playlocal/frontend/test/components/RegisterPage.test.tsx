...
describe("RegisterPage", () => {
  ...
  it("shows error if age is not confirmed when submitting step 1", () => {
    ...
    fireEvent.change(screen.getByLabelText(/display name/i), { target: { value: "Hudson" } });
    fireEvent.change(screen.getByLabelText(/^email$/i), { target: { value: "h@x.com" } });
  ...
  it("shows error if EULA is not accepted when submitting step 1", () => {
    ...
    fireEvent.click(screen.getByRole("checkbox", { name: /at least 13 years old/i }));

    fireEvent.click(screen.getByRole("button", { name: /continue/i }));

    expect(
      screen.getByText(/you must accept the terms of service and privacy policy/i),
    ).toBeInTheDocument();
  });
  ...
  it("moves to step 2 when age + EULA are accepted", () => {
    ...
    expect(screen.getByRole("heading", { name: /tell us about you/i })).toBeInTheDocument();
  ...
  it("step 2 shows error if intensity not selected", () => {
    ...
    expect(screen.getByText(/please select your play intensity/i)).toBeInTheDocument();
  });
  ...
  it("step 2 shows error if availability not selected", () => {
    ...
    expect(screen.getByText(/please select at least one availability option/i)).toBeInTheDocument();
  });
  ...
  it("submits step 2: calls register and navigates to /discover on success", async () => {
    ...
    // Step 1 inputs
    fireEvent.change(screen.getByLabelText(/display name/i), { target: { value: "Hudson" } });
    fireEvent.change(screen.getByLabelText(/^email$/i), { target: { value: "hudson@x.com" } });
    fireEvent.change(screen.getByLabelText(/password/i), { target: { value: "password123" } });
    fireEvent.click(screen.getByRole("checkbox", { name: /at least 13 years old/i }));
    fireEvent.click(screen.getByRole("checkbox", { name: /i agree to the/i }));
    fireEvent.click(screen.getByRole("button", { name: /continue/i }));
