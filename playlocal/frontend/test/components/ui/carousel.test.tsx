...
function renderBasicCarousel(
  extra?: Partial<React.ComponentProps<typeof Carousel>>
) {
  return render(
    <Carousel {...extra}>
...
describe('Carousel', () => {
  ...
  test('passes through opts while overriding axis based on orientation', () => {
    renderBasicCarousel({
      ...
      opts: { loop: true, align: 'start' as any },
    });
  ...
  test('ArrowLeft/ArrowRight keydown on the carousel container triggers scrollPrev/scrollNext', () => {
    ...
    expect(leftEvent.preventDefault as any as jest.Mock).toHaveBeenCalledTimes(
      1
    );
    expect(rightEvent.preventDefault as any as jest.Mock).toHaveBeenCalledTimes(
      1
    );
