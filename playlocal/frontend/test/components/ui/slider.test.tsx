...
describe('Slider', () => {
  ...
  it('handles defaultValue', () => {
    const { rerender } = render(
      <Slider defaultValue={[25, 75]} data-testid="slider" />
    );
  ...
  it('renders track and range', () => {
    ...
    const slider = screen.getByTestId('slider');
    expect(
      slider.querySelector('[data-slot="slider-track"]')
    ).toBeInTheDocument();
    expect(
      slider.querySelector('[data-slot="slider-range"]')
    ).toBeInTheDocument();
  });
  ...
  it('renders single thumb for single value', () => {
    ...
    const slider = screen.getByTestId('slider');
    expect(slider.querySelectorAll('[data-slot="slider-thumb"]')).toHaveLength(
      1
    );
  });
});
