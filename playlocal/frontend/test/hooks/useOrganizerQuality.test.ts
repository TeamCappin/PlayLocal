...
describe('useOrganizerQuality hook', () => {
  ...
  it('should fetch OQS data with userId', async () => {
    ...
    (organizerQualityApi.getOqsSummary as jest.Mock).mockResolvedValue(
      mockOqsSummary
    );
    (organizerQualityApi.getOqsInfoCard as jest.Mock).mockResolvedValue(
      mockInfoCard
    );

    const { result } = renderHook(() =>
      useOrganizerQuality({ userId: 'user-123' })
    );
  ...
  it('should fetch OQS data without userId (current user)', async () => {
    (organizerQualityApi.getMyOqs as jest.Mock).mockResolvedValue(
      mockOqsResponse
    );
    (organizerQualityApi.getMyOqsInfoCard as jest.Mock).mockResolvedValue(
      mockInfoCard
    );
  ...
  it('should not auto-fetch when autoFetch is false', async () => {
    const { result } = renderHook(() =>
      useOrganizerQuality({ userId: 'user-123', autoFetch: false })
    );
  ...
  it('should handle info card fetch error gracefully', async () => {
    ...
    (organizerQualityApi.getOqsInfoCard as jest.Mock).mockRejectedValue(
      new Error('Info card error')
    );
  ...
  it('should handle summary fetch error gracefully', async () => {
    ...
    (organizerQualityApi.getOqsSummary as jest.Mock).mockRejectedValue(
...
describe('useOrganizerQualityHistory hook', () => {
  ...
  it('should fetch history with userId', async () => {
    ...
    const { result } = renderHook(() =>
      useOrganizerQualityHistory({ userId: 'user-123' })
    );
    ...
    expect(organizerQualityApi.getOqsHistory).toHaveBeenCalledWith(
      'user-123',
      0,
      10
    );
  ...
  it('should fetch history without userId (current user)', async () => {
    (organizerQualityApi.getMyOqsHistory as jest.Mock).mockResolvedValue(
      mockHistoryResponse
    );
  ...
  it('should handle fetch error', async () => {
    (organizerQualityApi.getOqsHistory as jest.Mock).mockRejectedValue(
      new Error('API Error')
    );
  ...
  it('should not auto-fetch when autoFetch is false', async () => {
    const { result } = renderHook(() =>
      useOrganizerQualityHistory({ userId: 'user-123', autoFetch: false })
    );
  ...
  it('should use custom pageSize', async () => {
    ...
    renderHook(() =>
      useOrganizerQualityHistory({ userId: 'user-123', pageSize: 20 })
    );
