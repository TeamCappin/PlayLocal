...
describe('useGameChat', () => {
  ...
  it('dedupes inbound by id (same id does not append twice)', async () => {
    ...
    await waitFor(() => {
      expect(
        result.current.messages.filter((m) => m.content === 'dup-by-id')
      ).toHaveLength(1);
    });
  });
  ...
  it('dedupes inbound by TTL key (same sender|content|createdAt even with different ids)', async () => {
    ...
    act(() => {
      lastSubscribeCallback?.({
        body: JSON.stringify(makeInbound({ ...base, messageId: 'id-1' })),
      });
      lastSubscribeCallback?.({
        body: JSON.stringify(makeInbound({ ...base, messageId: 'id-2' })),
      });
    });

    await waitFor(() => {
      expect(
        result.current.messages.filter((m) => m.content === 'dup-by-key')
      ).toHaveLength(1);
    });
  });
  ...
  it('sendMessage adds optimistic tmp-* then reconciles when inbound echoes clientMessageId', async () => {
    ...
    await waitFor(() => {
      expect(result.current.messages.some((m) => m.id.startsWith('tmp-'))).toBe(
        true
      );
    });
    ...
    await waitFor(() => {
      ...
      expect(
        result.current.messages.some((m) => m.id === 'server-real-1')
      ).toBe(true);
      expect(result.current.messages.some((m) => m.id.startsWith('tmp-'))).toBe(
        false
      );
  ...
  it('onStompError sets error and disconnected state', async () => {
    ...
    await waitFor(() =>
      expect(result.current.error).toBe('WebSocket/STOMP error.')
    );
    expect(result.current.connected).toBe(false);
  });
  ...
  it('reconciles optimistic message when server echoes with same clientMessageId', async () => {
    ...
    await waitFor(() => {
      expect(
        result.current.messages.some((m) => m.id === 'server-echo-1')
      ).toBe(true);
      expect(
        result.current.messages.filter((m) => m.content === 'hello')
      ).toHaveLength(1);
    });
  });
