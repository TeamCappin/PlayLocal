...
function errorTextResponse(status: number, statusText: string, bodyText: string): Response {
  return {
    ok: false,
    status,
    statusText,
...
describe("apiFetch core behaviors", () => {
  ...
  it("adds Authorization header when token is set", async () => {
    const fetchMock = jest.fn().mockResolvedValue(jsonResponse({ gameId: "g-1" }));
    (globalThis as any).fetch = fetchMock;
  ...
  it('returns undefined when content-length is "0"', async () => {
    const fetchMock = jest.fn().mockResolvedValue(contentLengthZeroJsonResponse());
    (globalThis as any).fetch = fetchMock;
  ...
  it("returns undefined when content-type is not application/json", async () => {
    const fetchMock = jest.fn().mockResolvedValue(nonJsonContentTypeResponse());
    (globalThis as any).fetch = fetchMock;
...
describe("gamesApi query building + join body", () => {
  ...
  it("join sends POST with undefined body when joinRequest is omitted", async () => {
    const fetchMock = jest.fn().mockResolvedValue(jsonResponse({ joinStatus: "CONFIRMED" }));
    (globalThis as any).fetch = fetchMock;
...
describe("photosApi endpoints", () => {
  ...
  it("requestUploadSlot calls POST with JSON body", async () => {
    const fetchMock = jest.fn().mockResolvedValue(
      jsonResponse({ mediaId: "m1", storageKey: "k1", uploadUrl: "https://x" }),
    );
    (globalThis as any).fetch = fetchMock;
    ...
    expect(res).toEqual({ mediaId: "m1", storageKey: "k1", uploadUrl: "https://x" });
    expect(fetchMock).toHaveBeenCalledWith(
