...
export function useOrganizerQuality(options: UseOrganizerQualityOptions = {}) {
  ...
  const fetchOqs = useCallback(async () => {
    ...
    try {
      ...
      const method = userId
        ? organizerQualityApi.getOqs
        : organizerQualityApi.getMyOqs;
      if (typeof method !== 'function') {
        throw new Error(
          `organizerQualityApi.${userId ? 'getOqs' : 'getMyOqs'} is not available`
        );
      }
  ...
  const fetchInfoCard = useCallback(async () => {
    try {
      ...
      const method = userId
        ? organizerQualityApi.getOqsInfoCard
        : organizerQualityApi.getMyOqsInfoCard;
      if (typeof method !== 'function') {
        return;
      }
      const response = userId
 ...
 */
export function useOrganizerQualityHistory(
  options: UseOrganizerQualityOptions & { pageSize?: number } = {}
) {
  ...
  const fetchHistory = useCallback(
    async (pageNum: number = 0) => {
      ...
      try {
        ...
        const method = userId
          ? organizerQualityApi.getOqsHistory
          : organizerQualityApi.getMyOqsHistory;
        if (typeof method !== 'function') {
          throw new Error(
            `organizerQualityApi.${userId ? 'getOqsHistory' : 'getMyOqsHistory'} is not available`
          );
        }
 ...
 */
export function getConfidenceLevelInfo(level: string): {
  label: string;
  color: string;
  icon: string;
} {
 ...
 */
export function formatOqsReason(reason: string): {
  label: string;
  color: string;
} {
  switch (reason) {
