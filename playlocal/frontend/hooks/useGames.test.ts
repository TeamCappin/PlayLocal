...
const mockRoster = {
  confirmed: [{ participationId: 'p1', userId: 'u1', displayName: 'Alice', role: 'PARTICIPANT', joinStatus: 'CONFIRMED' }],
  waitlisted: [],
  maxPlayers: 10,
  spotsAvailable: 9,
} as any;
...
describe('usePastGamesByUserNeedingAttendanceUpdate', () => {
  ...
  it('fetches games on mount', async () => {
    ...
    const { result } = renderHook(() => usePastGamesByUserNeedingAttendanceUpdate());
  ...
  it('sets error when fetch fails', async () => {
    mockGetPastByUserNeedingAttendanceUpdate.mockRejectedValue(new Error('Unauthorized'));
    ...
    expect(result.current.error).toBe('Failed to load past games needing attendance update');
    expect(result.current.games).toEqual([]);
  });
  ...
  it('refetch reloads games', async () => {
    ...
    mockGetPastByUserNeedingAttendanceUpdate.mockResolvedValue([mockGame, { ...mockGame, gameId: 'g2' } as any]);
