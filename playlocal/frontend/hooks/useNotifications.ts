...
function toUINotification(notification: NotificationDto): UINotification {
  ...
  if (payload.title !== undefined && typeof payload.title !== 'string') {
    console.warn(
      `[useNotifications] Unexpected type for payload.title: ${typeof payload.title}`,
      payload
    );
  }
  if (payload.message !== undefined && typeof payload.message !== 'string') {
    console.warn(
      `[useNotifications] Unexpected type for payload.message: ${typeof payload.message}`,
      payload
    );
  }
  if (payload.link !== undefined && typeof payload.link !== 'string') {
    console.warn(
      `[useNotifications] Unexpected type for payload.link: ${typeof payload.link}`,
      payload
    );
  }
  if (payload.gameId !== undefined && typeof payload.gameId !== 'string') {
    console.warn(
      `[useNotifications] Unexpected type for payload.gameId: ${typeof payload.gameId}`,
      payload
    );
  }
  ...
  const title =
    ...
    buildNotificationTitle(type);
  const message =
    (typeof payload.message === 'string' ? payload.message : '') ||
    (typeof payload.gameTitle === 'string'
      ? `Update for ${payload.gameTitle}`
      : title);
  const link =
...
export function useNotifications() {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener('playlocal-refresh-notifications', handler);
  }, [fetchNotifications]);
